...
Command Center (Centralized Dashboard)
...
A vulnerable virtual machine (Metasploitable2) was set up as a sandboxed target.
An Nmap scan was initiated from the platform, which successfully identified the vsftpd 2.3.4 service.
The corresponding Metasploit module (exploit/unix/ftp/vsftpd_234_backdoor) was linked to the vulnerability in the database.
The "Test Exploit" button was triggered from the scan report page, scheduling a Celery task.
The task successfully connected to the Metasploit RPC service (msfrpcd), configured the exploit, and launched it against the target.
Verification confirmed that a command shell session was opened, achieving root access on the target machine. This demonstrates the platform's ability to seamlessly pivot from reconnaissance to successful exploitation.
...
Email Sent: Confirms delivery.
Email Opened: A 1x1 tracking pixel logs when the email is viewed.
Link Clicked: A unique tracking URL logs the click before redirecting the user.
Data Submitted: A credential harvester endpoint captures submitted data.
...
Start the Metasploit RPC Service: msfrpcd -P your_password -S
Start the Redis Server: sudo service redis-server start
Run the Celery Worker: celery -A defense_platform worker -l info
